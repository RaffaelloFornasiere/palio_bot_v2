...
# All data endpoints live under /api — keeps the root path free for the React app.
...
# Project root: src/palio_bot/api/api_server.py → .../<project>/
PROJECT_ROOT = Path(__file__).resolve().parents[3]
PALIO_FILE_PATH = PROJECT_ROOT / "data" / "palio.json"
DATA_DIR_PATH = PROJECT_ROOT / "data"
REACT_BUILD_PATH = PROJECT_ROOT / "website" / "build"
...
@api.get("/palio", response_model=PalioData, operation_id="get_palio_data")
...
@api.get("/leaderboard", response_model=Leaderboard, operation_id="get_leaderboard_data")
...
@api.get("/palio_games_status", response_model=PalioGamesStatus, operation_id="get_palio_games_status")
...
@api.get("/years", response_model=AvailableYearsResponse, operation_id="get_available_years")
